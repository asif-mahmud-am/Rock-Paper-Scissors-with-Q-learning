...
async def play(player_action: int):
    ...
    if player_action not in [0, 1, 2]:
        return JSONResponse({"error": "Invalid move! Please enter 0, 1, or 2."}, status_code=400)
    
    opponent_action = rps.get_action(state)
    if player_action == opponent_action:
        result = "It's a tie!"
        reward = 0
    elif (player_action == 0 and opponent_action == 1) or (player_action == 1 and opponent_action == 2) or (
